...
- **Built-in FTP server** — upload and download files wirelessly from your PC
...
IconSwap includes a built-in FTP server so you can transfer files wirelessly between your PC and SD card without ejecting it.

### Starting the FTP server

Press **X** on the game list screen. The FTP screen shows your Switch's IP address and connection details.

### Connecting

| Setting | Value |
|---|---|
| Host | IP shown on screen (e.g. `192.168.0.55`) |
| Port | `5000` |
| Username | anything |
| Password | anything |
| Mode | **Passive (PASV)** — required |

FileZilla, WinSCP, Cyberduck, and most FTP clients work. Make sure passive mode is enabled.

### What you can do

- **Upload** images directly to any folder on the SD card
- **Download** files from the SD card to your PC
- Create, rename, and delete files and folders
- Full SD card is accessible — FTP root `/` maps to `sdmc:/`

The FTP server runs on its own thread so the UI stays responsive while transferring files. Press **B** to go back to the game list — the server keeps running in the background.
...
| B | Back |
| X | Open FTP server |
| Y | Remove custom icons from selected game |
| − | Reboot to payload |
| + | Exit |
...
1. Copy your image files anywhere on your SD card (or use FTP to upload them wirelessly)
...
Scroll to the game and press **Y**. This deletes both `icon.jpg` and `icon174.jpg` for that title.
...
- UTF-8 filenames are fully supported throughout
